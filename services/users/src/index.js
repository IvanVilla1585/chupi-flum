...
const { schemaUser } = require('./lib/validator');
...
const hash = HttpHash();
const sendGrid = new Email(config.sendGridApiKey);
...
/*
  ...
  */
hash.set('POST /', compose(schemaUser, db)(async (req, res, params) => {

  const { Model } = req;
  const data = await json(req);
  const resp = await new Model(data).save();

  if (resp.errors) throw new Error(resp.messages);

	const token = jwt.sign({email: resp.email}, config.secretToken, {expiresIn: 3600});
	const url = `http://localhost:4200/usuario/registro-password/${token}`;
	const name = `${resp.name} ${resp.lastName}`;

  sendGrid.confirmInscription(resp.email, 'ivan_villa23141@elpoli.edu.co', {name, url});
